...
| 스크린샷 1 | 1280×800 | 초기화면 (페이지 정보 로드) | **필요** |
| 스크린샷 2 | 1280×800 | 다운로드 진행 중 (진행 바) | **필요** |
| 스크린샷 3 | 1280×800 | 다운로드 완료 (결과 메시지) | **필요** |
...
- [ ] **짧은 설명** (132자 이내)
  ```
  네이버페이·쿠팡 결제내역을 Excel/CSV로 한번에 다운로드하세요.
  ```

- [ ] **상세 설명** (4000자 이내)
  ```
  📊 Eolma Exporter - 결제내역 다운로드 및 가계부 관리 도구

  Eolma Exporter는 네이버페이와 쿠팡의 결제/주문내역을 한 곳에서 
  ...
  • 전체 다운로드: 모든 거래내역 한번에 수집 (다중 페이지 자동 처리)

  🔹 다양한 파일 형식
  • Excel(.xlsx): 스프레드시트로 즉시 분석
  • CSV: 다른 도구와의 호환성
  ...
  🔗 eolma 가계부 서비스 연동
  eolma 가계부 서비스와 함께 사용하면 수집한 거래내역을 
  바로 개인 가계부에 등록하여 효율적인 재정 관리가 가능합니다.

  💡 사용 방법
  1. 네이버페이 또는 쿠팡 결제/주문내역 페이지 방문
  2. Eolma Exporter 아이콘 클릭
  3. 원하는 범위와 형식 선택
  4. 다운로드 완료!

  🔒 개인정보 보호
  • 사용자 브라우저에서만 동작 (서버 전송 없음)
  • 다운로드 후 즉시 삭제되므로 개인정보 안전
  • 개인정보처리방침 준수

  지금 바로 설치하고 거래내역 관리를 시작하세요!
  ```
...
**v0.1.0부터 한국어 및 영어를 완전히 지원**합니다.

사용자의 브라우저 언어 설정에 따라 자동으로 언어가 선택됩니다:
- 🇰🇷 **한국어** (기본값 / 한국 사용자)
- 🇬🇧 **영어** (영어권 사용자)
